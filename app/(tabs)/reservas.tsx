...
export default function Reservas() {
        ...
        return (
            <Screen>
                <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptySubtext}>Por favor, inicia sesión para ver tus reservas.</Text>
                </View>
            </Screen>
        );
    }
    ...
    const obtenerReservas = async () => {
        try {
            const response = await fetch(`http://192.168.100.147:4000/api/reserva/usuario/${user.id}`);
    ...
    const renderReservaItem = ({ item }: { item: Reserva }) => (
        <Pressable style={styles.reservaCard}>
            <View style={styles.cardHeader}>
                <Image
                    source={{ 
                        uri: item.escenario?.imagenUrl || 'https://via.placeholder.com/80x60?text=Sin+Imagen' 
                    }}
            ...
            <View style={styles.cardBody}>
                <View style={styles.infoRow}>
                    ...
                    <Text style={styles.infoText}>
                        {formatearFecha(item.fecha)} • {formatearHora(item.horaInicio)} - {formatearHora(item.horaFin)}
                    </Text>
                </View>

                <View style={styles.infoRow}>
                    {icons.escenarios({ color: '#6B7280', size: 16 })}
                    <Text style={styles.infoText}>{item.escenario?.direccion || 'Sin dirección'}</Text>
                </View>

                <View style={styles.infoRow}>
                    {icons.reportes({ color: '#6B7280', size: 16 })}
                    <Text style={styles.infoText}>Tipo: {item.escenario?.tipo || 'Sin tipo'}</Text>
                </View>
            </View>
    ...
    // ✅ Agregar validación para estado null
    const reservasCompletadas = reservas.filter(r => 
        r.estado?.nombre && 
        (r.estado.nombre.toLowerCase() === 'completada' || r.estado.nombre.toLowerCase() === 'activa')
    ).length;
