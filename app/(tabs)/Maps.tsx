import { StyleSheet, View, Image, Text, FlatList, TextInput, TouchableOpacity } from 'react-native';
...
type Escenario = {
  id: number;
  nombre: string;
  latitud: number | string; // Puede venir como string de la API
  longitud: number | string; // Puede venir como string de la API
  imagen: string;
  direccion?: string;
  tipo?: string;
  capacidad?: string;
...
export default function Maps() {
  ...
  useEffect(() => {
    if (searchText.trim() === '') {
      ...
      const filtered = escenarios.filter(escenario =>
        ...
        escenario.direccion?.toLowerCase().includes(searchText.toLowerCase()) ||
  ...
  const navigateToEscenario = (escenario: Escenario) => {
    ...
    const lat = typeof escenario.latitud === 'string' ? parseFloat(escenario.latitud) : escenario.latitud;
    const lng = typeof escenario.longitud === 'string' ? parseFloat(escenario.longitud) : escenario.longitud;
    
    // Verificar que sean números válidos
    if (isNaN(lat) || isNaN(lng)) {
      console.warn('Coordenadas inválidas para el escenario:', escenario.nombre);
      return;
    }
    ...
    >
      ...
      <View style={styles.escenarioContent}>
        ...
        <View style={styles.escenarioDetails}>
          ...
          <Text style={styles.escenarioDetailText}>Capacidad: {item.capacidad}</Text>
        </View>
      </View>
