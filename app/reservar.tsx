...
import { Text, StyleSheet, View, TouchableOpacity, Platform, Alert, ScrollView } from "react-native";
...
export default function Reservar() {
    ...
    const obtenerPrecioEscenario = async () => {
        try {
            ...
            const response = await fetch(`http://192.168.100.147:4000/api/escenario/${escenarioIdNumber}`);
            
            console.log('=== API RESPONSE DEBUG ===');
            console.log('URL llamada:', `http://192.168.100.147:4000/api/escenario/${escenarioIdNumber}`);
    ...
    const handleReservar = async () => {
        ...
        if (duracionHoras < 0.5) {
            Alert.alert('Error', 'La reserva debe tener una duración mínima de 30 minutos');
            return;
        }
        ...
        // ✅ Corregir el tipo de escenarioId
        const escenarioIdNumber = Array.isArray(escenarioId) 
            ? parseInt(escenarioId[0]) 
            : parseInt(escenarioId);

        try {
            // Solo enviar los campos que espera la API (sin precioTotal)
            const res = await ReservarEscenario({
            ...
            if (res && res.res && res.res.ok) {
                ...
                Alert.alert(
                    'Reserva Exitosa', 
                    `¡Reserva realizada correctamente!\n\nDetalles:\n• Fecha: ${selectedDate.toLocaleDateString()}\n• Hora: ${horaInicio} - ${horaFin}\n• Duración: ${formatearDuracion(duracionHoras)}\n• Precio Total: $${precioTotal.toLocaleString()}`
                );
            } else {
                Alert.alert('Error', 'Error al realizar la reserva. Por favor, intenta de nuevo.');
            }
        } catch (error) {
    ...
    return (
        <View style={styles.container}>
           <ScrollView>
            {user && (
                <Text style={styles.title}>Reservar Escenario </Text>
            )}
            ...
            <View style={styles.priceContainer}>
                ...
                <View style={styles.priceRow}>
                    ...
                    <Text style={styles.priceValue}>{formatearDuracion(duracionHoras)}</Text>
                </View>
                
                <View style={styles.priceRow}>
                    <Text style={styles.priceLabel}>Precio por hora:</Text>
                    <Text style={styles.priceValue}>${precioPorHora.toLocaleString()}</Text>
                </View>
                
                <View style={[styles.priceRow, styles.totalRow]}>
                    <Text style={styles.totalLabel}>Precio Total:</Text>
                    <Text style={styles.totalValue}>${precioTotal.toLocaleString()}</Text>
                </View>
            </View>
